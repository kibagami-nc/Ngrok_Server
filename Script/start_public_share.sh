...
# Répertoires
WEB_DIR="$HOME/Ngrok_Web"
SHARE_DIR="$HOME/Public"

# Port fixe pour Flask / Ngrok
PORT=8765

# Créer le dossier web s’il n’existe pas
mkdir -p "$WEB_DIR"

# Créer l’app Flask dans Ngrok_Web
...
# Récupérer l’URL publique Ngrok + A COMPLETER EN BAS SELON IP ET PORT
LINK=$(curl --silent http://VOTRE IP ET PORT/api/tunnels | grep -oP '"public_url":"\Khttps:[^"]+')

# Écrire le lien dans Public/link.txt
echo "Lien d'accès au dossier Public :" > "$SHARE_DIR/link.txt"
echo "$LINK" >> "$SHARE_DIR/link.txt"
